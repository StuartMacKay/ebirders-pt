...
def test_list_view__is_not_paginated(db_no_rollback, client):
    url = reverse("species:list")
    response = client.get(url)
    view = response.context["view"]
    assert view.paginate_by is None
...
def test_list_view__filter_by_category(db_no_rollback, client, category):
    url = reverse("species:list")
    response = client.get(url, data={"category": category})
    objects = response.context["object_list"]
    assert objects.count() != 0
    for obj in objects:
        assert obj.species.category == category
