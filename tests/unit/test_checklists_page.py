...
def test_checklists_page__filtered_by_country(
...
def test_checklists_page__filtered_by_state(
...
def test_checklists_page__filtered_by_county(
...
def test_checklists_page__filtered_by_observer(
...
def test_checklists_page__filtered_by_start_date(
...
def test_checklists_page__filtered_by_end_date(
...
def test_checklists_page__filtered_by_date_range(
...
def test_checklists_page__filtered_by_hotspot(
    db_no_rollback, client, url, filter_by_hotspot
):
    response = client.get("%s?%s" % (url, filter_by_hotspot))
    assert has_pages(response)


def test_checklists_page__ordered_by_species_count(
    db_no_rollback, client, url, ordered_by_species_count
):
    response = client.get("%s?%s" % (url, ordered_by_species_count))
    assert has_pages(response)
